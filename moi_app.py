...
def _get_secret(name: str, default=None):
    try:
        if hasattr(st, "secrets") and name in st.secrets:  # type: ignore[attr-defined]
...
if not st.session_state["authenticated"]:
    ...
    if not APP_PASSWORD:
        st.warning(
            "MOI_PASSWORD not found in secrets/.env. Please set a secure password."
        )
...
MOI_SCALES = {
    "Year": [
        ("Remarkable", 2_000_000, 0.20, 11_000, 900, 10_000_000),
        ("Excellent", 1_520_000, 0.19, 10_000, 780, 8_000_000),
        ("Great", 1_080_000, 0.18, 9_000, 660, 6_000_000),
        ("Good", 850_000, 0.17, 8_000, 600, 5_000_000),
        ("Average", 640_000, 0.16, 7_000, 540, 4_000_000),
        ("Poor", 450_000, 0.15, 6_000, 480, 3_000_000),
    ],
    "Month": [
        ("Remarkable", 166_667, 0.20, 11_000, 75, 833_333),
        ("Excellent", 126_667, 0.19, 10_000, 65, 666_667),
        ("Great", 90_000, 0.18, 9_000, 55, 500_000),
        ("Good", 70_833, 0.17, 8_000, 50, 416_667),
        ("Average", 53_333, 0.16, 7_000, 45, 333_333),
        ("Poor", 37_500, 0.15, 6_000, 40, 250_000),
    ],
    "Week": [
        ("Remarkable", 38_462, 0.20, 11_000, 34, 192_308),
        ("Excellent", 29_231, 0.19, 10_000, 28, 153_846),
        ("Great", 20_769, 0.18, 9_000, 23, 115_385),
        ("Good", 16_346, 0.17, 8_000, 20, 96_154),
        ("Average", 12_308, 0.16, 7_000, 17, 76_923),
        ("Poor", 8_654, 0.15, 6_000, 14, 57_692),
    ],
    "Day": [
        ("Remarkable", 5_479, 0.20, 11_000, 7, 27_397),
        ("Excellent", 4_164, 0.19, 10_000, 6, 21_918),
        ("Great", 2_959, 0.18, 9_000, 5, 16_438),
        ("Good", 2_329, 0.17, 8_000, 4, 13_699),
        ("Average", 1_753, 0.16, 7_000, 4, 10_959),
        ("Poor", 1_233, 0.15, 6_000, 3, 8_219),
    ],
}
...
def class_moi(
    row,
    scale,
    majority_required: int = 3,
    reinforce_revenue: bool = True,
):
...
def apply_filters(
    df: pd.DataFrame, exclude_sundays: bool, paid_only: bool, exclude_neg: bool
) -> pd.DataFrame:
...
def fetch_daily_activity(
    dstart: date, dend: date, usernames: list[str] | None
) -> pd.DataFrame:
    """
    ...
    start_ts = (
        pd.Timestamp(dstart).tz_localize("America/Bogota").tz_convert("UTC")
    )
    end_ts = (
        (pd.Timestamp(dend) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1))
    ...
    df["created_at"] = (
        pd.to_datetime(df["created_at"], utc=True, errors="coerce").dt.tz_convert(
            "America/Bogota"
        )
    )
    ...
    g = (
        ...
        .agg(
            reached=("clients_reached_out", "sum"),
...
def agg_by_rep(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return g.sort_values(["revenue_sum", "orders"], ascending=[False, False]).reset_index(
        drop=True
    )
...
with st.sidebar:
    ...
    targets_mode = st.radio(
        ...
        help=(
            "Calendar: × number of calendar periods in range · Observed: × periods with data"
        ),
    )
...
def goal_per_period(
    ...
    if use_observed:
        def _obs(gr):
            mask = (df_scope[col_date] >= pd.Timestamp(base_d)) & (
                df_scope[col_date] <= pd.Timestamp(end_d)
            )
...
out_interleaved = pd.DataFrame(
    rows,
    columns=["SALES_REP", "REVENUE_SUM", "PROFIT_SUM", "PROFIT_PCT", "ORDERS", "AOV"],
)
...
sty = sty.set_properties(**{"white-space": "normal"}).set_table_styles(
    [
...
def meter(title: str, actual: float, goal: float):
    ...
    <div style="background:#eee;border-radius:10px;overflow:hidden;height:22px;border:1px solid #ddd;">
        <div style="width:{pct*100:.4f}%;height:100%; background:linear-gradient(90deg,#B7B7B7,#7F7F7F,#E06666,#A80E0E,#C00000); transition:width .4s ease"></div>
    </div>
...
with c1:
    meter(
        f"Day · {pd.to_datetime(dS).date()} → {pd.to_datetime(dE).date()}",
        actual_day,
        goal_day,
    )
    meter(
        f"Month · {pd.to_datetime(mS).date()} → {pd.to_datetime(mE).date()}",
        actual_month,
        goal_month,
    )
with c2:
    meter(
        f"Week · {pd.to_datetime(wS).date()} → {pd.to_datetime(wE).date()}",
        actual_week,
        goal_week,
    )
    meter(
        f"Year · {pd.to_datetime(yS).date()} → {pd.to_datetime(yE).date()}",
        actual_year,
        goal_year,
    )
...
rep_options = (
    sorted(df_f[COL_REP].dropna().astype(str).unique().tolist()) if not df_f.empty else []
)
...
if selected_rep != "(All)" and not user_map_df.empty:
    usernames_for_rep = (
        user_map_df.loc[user_map_df["full_name"] == selected_rep, "username"]
        .astype(str)
        .unique()
        .tolist()
    )
    if not usernames_for_rep:  # fallback por si sales_rep ya fuese username
...
sales_daily["profit_pct"] = sales_daily.apply(
    lambda r: _safe_div(r["profit_sum"], r["revenue_sum"]), axis=1
)

# Outer join por fecha (para no perder días sin ventas o sin actividad)
comb = (
    pd.merge(sales_daily, activity_daily, on="d", how="outer", validate="1:1")
    .fillna(
        {
            "orders": 0,
            "revenue_sum": 0,
...
if (comb["engaged"] > comb["reached"]).any() or (comb["closed"] > comb["reached"]).any():
    st.warning(
        "Some days show Engaged/Closed > Reached. Input was capped for display; please review daily_metrics entries."
    )
